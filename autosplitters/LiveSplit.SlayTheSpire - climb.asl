...
update
{
    ...
    while((line = vars.reader.ReadLine()) != null){ //Read the log until its end
        ...
        if(line.Contains("Generating seeds")){
            ...
            vars.command = "SPLIT";
            return true;
        } else if (timer.CurrentPhase == TimerPhase.Running & System.Text.RegularExpressions.Regex.IsMatch(line, @"(Achievement Unlocked: )(RUBY|EMERALD|SAPPHIRE|AMETHYST|ASCEND_20)")){
            vars.command = "SPLIT";
            return true;
        }
	}
