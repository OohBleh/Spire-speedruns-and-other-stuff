...
Last modified by OohBleh in May 2022
 */
...
update
{
    ...
    while((line = vars.reader.ReadLine()) != null){ //Read the log until its end
        ...
        if(line.Contains("UPDATING DEFAULT SLOT: ")){
            vars.command = "START";
            return true;
        } else if (timer.CurrentPhase == TimerPhase.Running & line.Contains("ASCENSION LEVEL IS NOW: ")){
            vars.command = "SPLIT";
            return true;
        } else if (timer.CurrentPhase == TimerPhase.Running & System.Text.RegularExpressions.Regex.IsMatch(line, @"(Achievement Unlocked: )(EMERALD|SAPPHIRE|AMETHYST|LUCKY_DAY|EMERALD_PLUS|SAPPHIRE_PLUS|AMETHYST_PLUS|POWERFUL|ASCEND_20)")){
            vars.command = "SPLIT";
            return true;
        }
	}
